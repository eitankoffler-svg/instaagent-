...
export async function POST(request: NextRequest) {
  ...
  if (!accountId) {
    ...
    if (!first) {
      return NextResponse.json({ error: "No Instagram account connected. Add one first." }, { status: 400 });
    }
    const contentType = (["reel", "story", "carousel", "ad"] as const).includes(body.contentType) ? body.contentType : "reel";
